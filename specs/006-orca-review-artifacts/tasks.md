...
- [x] T001 Review `specs/006-orca-review-artifacts/spec.md`, `specs/006-orca-review-artifacts/plan.md`, and `specs/006-orca-review-artifacts/contracts/` for implementation readiness
- [x] T002 [P] Review `commands/code-review.md`, `commands/cross-review.md`, `commands/pr-review.md`, `commands/self-review.md`, and `templates/review-template.md` against the planned two-layer artifact model
- [x] T003 [P] Document any current `review.md` assumptions in `README.md` that must be updated during implementation
...
- [x] T004 Finalize the preferred artifact set in `specs/006-orca-review-artifacts/contracts/review-artifact-files.md`
- [x] T005 Finalize stage ownership rules in `specs/006-orca-review-artifacts/contracts/review-artifact-ownership.md`
- [x] T006 Finalize summary/index responsibilities in `specs/006-orca-review-artifacts/contracts/review-summary-index.md`
- [x] T007 Align `specs/006-orca-review-artifacts/data-model.md` with the finalized artifact and ownership contracts
...
- [x] T008 [US1] Update `commands/code-review.md` so `speckit.orca.code-review` owns `review-code.md` as its primary durable artifact
- [x] T009 [US1] Update `commands/cross-review.md` so `speckit.orca.cross-review` owns `review-cross.md` as its primary durable artifact
- [x] T010 [US1] Update `commands/pr-review.md` so `speckit.orca.pr-review` owns `review-pr.md` as its primary durable artifact
- [x] T011 [US1] Update `commands/self-review.md` to reinforce that `self-review.md` remains a separate process artifact rather than part of the implementation-review bundle
- [x] T012 [US1] Manually verify quickstart Scenarios 1 through 4 using `specs/006-orca-review-artifacts/quickstart.md`
...
- [x] T013 [US2] Update `templates/review-template.md` to reflect `review.md` as an umbrella summary/index instead of the only authoritative review record
- [x] T014 [US2] Add explicit stage-artifact references and status-summary behavior to `templates/review-template.md`
- [x] T015 [US2] Update `README.md` to describe the new review artifact architecture and how `review.md` relates to stage artifacts
- [x] T016 [US2] Manually verify quickstart Scenario 5 using `specs/006-orca-review-artifacts/quickstart.md`
...
- [x] T017 [US3] Add the stage-detection contract to `specs/006-orca-review-artifacts/contracts/review-artifact-files.md` so later consumers can distinguish `present`, `missing`, and `summary_only`
- [x] T018 [US3] Clarify in `specs/006-orca-review-artifacts/data-model.md` that review-artifact fields are conceptual unless a later file-encoding contract is introduced
- [x] T019 [US3] Update `commands/self-review.md` so self-review loads `review-code.md`, `review-cross.md`, and `review-pr.md` alongside `review.md` when they exist
- [x] T020 [US3] Manually verify that a later consumer can determine review-stage presence from the documented artifact rules without treating `review.md` as the only source of truth
...
- [x] T021 [US4] Tighten `commands/cross-review.md` so cross-review output and append behavior target `review-cross.md` distinctly from code review output
- [x] T022 [US4] Tighten `commands/pr-review.md` so PR lifecycle, comment disposition, and post-merge evidence target `review-pr.md` distinctly from code-review findings
- [x] T023 [US4] Update `specs/006-orca-review-artifacts/contracts/review-artifact-files.md` if implementation decisions refine the distinction between `review-cross.md` and `review-pr.md`
- [x] T024 [US4] Manually verify that cross-review and PR-review evidence remain distinguishable in the resulting artifact set
...
- [x] T025 [P] Update `specs/006-orca-review-artifacts/contracts/review-summary-index.md` and `specs/006-orca-review-artifacts/contracts/review-artifact-ownership.md` if implementation details shifted
- [x] T026 [P] Run a command-doc consistency pass across `commands/code-review.md`, `commands/cross-review.md`, `commands/pr-review.md`, `commands/self-review.md`, and `README.md`
- [x] T027 Run the full quickstart validation flow in `specs/006-orca-review-artifacts/quickstart.md`, including legacy-migration coverage, and record the verification evidence in `specs/006-orca-review-artifacts/tasks.md` or commit notes
- [x] T028 Run final verification via `git diff --check` and `bash -n` on any touched shell wrappers if runtime glue was added
...
- Verified command-doc and template ownership alignment with targeted `rg`
  scans across `commands/`, `README.md`, and `templates/`
- Verified `review.md` now acts as summary/index while `review-code.md`,
  `review-cross.md`, and `review-pr.md` are referenced as stage artifacts
- Verified legacy migration coverage exists in `quickstart.md` Scenario 6
- Ran `git diff --check`
- No shell runtime files changed, so `bash -n` was not needed for this feature
